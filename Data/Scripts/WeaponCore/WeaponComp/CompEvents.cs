            ...
            try
            {
                ...
                if (Debug)
                {
                    foreach (var weapon in Platform.Weapons)
                    {
                        ...
                        stringBuilder.Append($"\nMagSize: {weapon.System.EnergyMagSize} - CurrentCharge: {State.Value.CurrentCharge}({weapon.State.Sync.CurrentCharge})");
                        stringBuilder.Append($"\nChargeTime: {weapon.Timings.ChargeUntilTick}({weapon.Comp.Ai.Session.Tick}) - Delay: {weapon.Timings.ChargeDelayTicks}");
                        stringBuilder.Append($"\nCharging: {weapon.State.Sync.Charging}({weapon.System.MustCharge}) - Delay: {weapon.Timings.ChargeDelayTicks}");
                    }
                }
            }
