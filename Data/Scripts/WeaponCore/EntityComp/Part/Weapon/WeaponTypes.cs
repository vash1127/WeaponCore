        ...
        internal class ParallelRayCallBack
        {
            ...
            public void NormalShootRayCallBack(IHitInfo hitInfo)
            {
                ...
                if (unexpectedHit)
                {
                    ...
                    if (topAsGrid == null)
                        return;
                    if (Weapon.Target.TargetEntity != null && Weapon.Comp.IsBlock && (topAsGrid.IsSameConstructAs(Weapon.Comp.Ai.GridEntity) || !topAsGrid.DestructibleBlocks || topAsGrid.Immune || topAsGrid.GridGeneralDamageModifier <= 0))
                    {
                        var hitPos = Weapon.Target.TargetEntity.PositionComp.WorldAABB.Center;
                        Vector3D pos; 
