...
namespace WeaponCore.Support
{
    ...
    {
        internal static void AcquireTarget(Unit w, bool attemptReset, MyEntity targetGrid = null)
        {
            //w.HitOther = false;
            ...
            else
            {
                ...
                if (w.Comp.Session.PlayerDummyTargets.TryGetValue(w.Comp.Data.Repo.Base.State.PlayerId, out dummyTarget) &&  Unit.CanShootTarget(w, ref dummyTarget.Position, dummyTarget.LinearVelocity, dummyTarget.Acceleration, out predictedPos))
                {
                    w.Target.SetFake(w.Comp.Session.Tick, predictedPos);
                    if (w.ActiveAmmoDef.ConsumableDef.Trajectory.Guidance != GuidanceType.None || w.Comp.IsBlock && !w.MuzzleHitSelf())
        ...
        private static void AcquireTopMostEntity(Unit w, out TargetType targetType, bool attemptReset = false, MyEntity targetGrid = null)
        {
            var comp = w.Comp;
                ...
                for (int x = 0; x < adjTargetCount; x++)
                {
                    ...
                    if (info.IsGrid) {
                        ...
                        if (!w.AiEnabled) {
                            ...
                            newCenter = w.System.Prediction != HardPointDef.Prediction.Off && (!w.ActiveAmmoDef.ConsumableDef.Const.IsBeamWeapon && w.ActiveAmmoDef.ConsumableDef.Const.DesiredProjectileSpeed > 0) ? Unit.TrajectoryEstimation(w, targetCenter, targetLinVel, targetAccel, out validEstimate) : targetCenter;
                        ...
                        else if (!Unit.CanShootTargetObb(w, info.Target, targetLinVel, targetAccel, out newCenter)) continue;
                    ...
                    Vector3D predictedPos;
                    if (!Unit.CanShootTarget(w, ref targetCenter, targetLinVel, targetAccel, out predictedPos, true, info.Target)) continue;
                    ...
                    if (w.LastHitInfo?.HitEntity != null && (!w.System.Values.HardPoint.Other.MuzzleCheck || w.Comp.IsBlock && !w.MuzzleHitSelf())) {
        ...
        private static bool AcquireBlock(CoreSystem system, Ai ai, Target target, TargetInfo info, Vector3D weaponPos, WeaponRandomGenerator wRng, RandomType type, ref BoundingSphereD waterSphere, Unit w = null, bool checkPower = true)
        {
        ...
        private static bool FindRandomBlock(CoreSystem system, Ai ai, Target target, Vector3D weaponPos, TargetInfo info, ConcurrentCachingList<MyCubeBlock> subSystemList, Unit w, WeaponRandomGenerator wRng, RandomType type, ref BoundingSphereD waterSphere, bool checkPower = true)
        {
            ...
            for (int i = 0; i < totalBlocks; i++)
            {
                ...
                if (turretCheck)
                {
                    ...
                    ai.Session.CanShoot++;
                    Vector3D predictedPos;
                    if (!Unit.CanShootTarget(w, ref blockPos, targetLinVel, targetAccel, out predictedPos)) continue;
        ...
        internal static bool GetClosestHitableBlockOfType(ConcurrentCachingList<MyCubeBlock> cubes, Ai ai, Target target, TargetInfo info, Vector3D currentPos, Vector3D targetLinVel, Vector3D targetAccel, ref BoundingSphereD waterSphere ,Unit w = null, bool checkPower = true)
        {
                ...
                if (test < minValue3) {
                    ...
                    if (best) {
                        ...
                        if (w != null && !(!w.IsTurret && w.ActiveAmmoDef.ConsumableDef.Trajectory.Smarts.OverideTarget)) {
                            ...
                            ai.Session.CanShoot++;
                            Vector3D predictedPos;
                            if (Unit.CanShootTarget(w, ref cubePos, targetLinVel, targetAccel, out predictedPos)) {
        ...
        internal static void AcquireProjectile(Unit w, out TargetType targetType)
        {
            var ai = w.Comp.Ai;
            var s = w.System;
            ...
            for (int x = 0; x < numOfTargets; x++)
            {
                ...
                Vector3D predictedPos;
                if (Unit.CanShootTarget(w, ref lp.Position, lp.Velocity, lp.AccelVelocity, out predictedPos))
                {
                    var needsCast = false;
                    ...
                    if (needsCast)
                    {
                        ...
                        if (hitInfo?.HitEntity == null && (!w.System.Values.HardPoint.Other.MuzzleCheck || w.Comp.IsBlock && !w.MuzzleHitSelf()))
                        {
                            double hitDist;
