...
namespace WeaponCore.Support
{
    public class EntityPool<T> where T : class, new()
    {
        ...
        private Func<string, T> _activator;
        ...
        public EntityPool(int baseCapacity, string modelName, Func<string, T> activator = null)
        {
            _modelName = modelName;
            _activator = activator ?? (s => new T());
            ...
            for (int index = 0; index < _baseCapacity; ++index)
                _unused.Enqueue(_activator(_modelName));
        }
        ...
        public bool AllocateOrCreate(out T item)
        {
            ...
            using (_activeLock.Acquire())
            {
                ...
                item = !flag ? _unused.Dequeue() : _activator(_modelName);
                _active.Add(item);
            }
            return flag;
        }
    ...
    /*
    public class ObjectsPool<T, TArg> where T : class
    {
        private SpinLockRef _activeLock = new SpinLockRef();
        private MyConcurrentQueue<T> _unused;
        private HashSet<T> _active;
        private HashSet<T> _marked;
        private Func<TArg, T> _activator;
        private int _baseCapacity;
        ...
        public ObjectsPool(int baseCapacity, Func<TArg, T> activator)
        {
            _activator = activator;//throw if this null
            _baseCapacity = baseCapacity;
            _unused = new MyConcurrentQueue<T>(_baseCapacity);
            _active = new HashSet<T>();
            _marked = new HashSet<T>();
        ...
        /// <summary>Returns true when new item was allocated</summary>
        public bool AllocateOrCreate(TArg arg, out T item)
        {
            bool flag = false;
            using (_activeLock.Acquire())
            {
                flag = _unused.Count == 0;
                item = !flag ? _unused.Dequeue() : _activator(arg);//but probably you need initializer...since you can't pass arg to old type
                _active.Add(item);
            }
            return flag;
        }
