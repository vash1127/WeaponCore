            ...
            if (!s.IsClient) {
                ...
                if (!ammo.AmmoDef.Const.EnergyAmmo)
                {
                    if (!s.IsCreative) {
                        ...
                        if (!weapon.PullingAmmo && !weapon.RemovingAmmo && weapon.CurrentAmmoVolume < 0.25f * weapon.System.MaxAmmoVolume && freeSpace > weapon.ActiveAmmoDef.AmmoDef.Const.MagVolume) {
