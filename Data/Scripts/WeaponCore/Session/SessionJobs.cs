        ...
        private void ProcessDbsCallBack()
        {
            try
            {
                ...
                for (int d = 0; d < DbsToUpdate.Count; d++)
                {
                    ...
                    db.DbReady = db.SortedTargets.Count > 0 || db.TargetAis.Count > 0 || Tick - db.LiveProjectileTick < 3600 || db.LiveProjectile.Count > 0 || db.ControllingPlayers.Count > 0 || db.FirstRun;
