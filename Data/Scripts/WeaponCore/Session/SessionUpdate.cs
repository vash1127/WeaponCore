                    ...
                    for (int i = 0; i < gridAi.Weapons.Count; i++) {
                        ...
                        using (comp.MyCube.Pin())
                        {
                            ...
                            for (int j = 0; j < comp.Platform.Weapons.Length; j++) {
                                ...
                                if (w.DelayCeaseFire && (directControl || !w.AiShooting || w.DelayFireCount++ > w.System.TimeToCeaseFire))
                                    w.DelayFireCount = 0;
                                ...
                                var validShootStates = !w.Comp.Set.Value.Overrides.ManualFire && comp.Gunner == Manual && w.Target.IsFakeTarget || w.State.ManualShoot == ShootOnce || w.AiShooting && w.State.ManualShoot == ShootOff;
                                ...
                                Log.Line($"            : validShootStates [1:{!w.Comp.Set.Value.Overrides.ManualFire && (comp.Gunner == Manual && w.Target.IsFakeTarget)}] [2:{w.State.ManualShoot == ShootOnce}] [3:{w.AiShooting && w.State.ManualShoot == ShootOff}]");
