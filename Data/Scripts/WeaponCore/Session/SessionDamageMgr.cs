    ...
    public partial class Session
    {
        internal void ProcessHits()
        {
            for (int x = 0; x < Hits.Count; x++)
            {
                ...
                var canDamage = !IsClient;
                
                //Log.Line($"canDamage: {canDamage} p.Info.ClientSent:{p.Info.ClientSent} p.Info.IsFiringPlayer: {p.Info.IsFiringPlayer} fixedFire: {fixedFire}");


                /*if (IsClient && p.Info.IsFiringPlayer && fixedFire)
                {
                    SendFixedGunHitEvent(p.Info.Target.FiringCube, p.Hit.Entity, info.HitList[0].HitPos ??Vector3.Zero, info.Direction, p.Velocity, p.Info.OriginUp, p.Info.MuzzleId, info.System.WeaponIdHash);
